...
import { pageNo, getCurrentOutlet } from '../Validator'
...
const Analysis = () => {
    ...
    const columns = [
        ...
        {
            name: 'Referrer',
            selector: row => row.referrer_email
        },
        {
            name: 'Referree',
            selector: row => row.referrer_email
    ...
    const getData = () => {
        setIsLoading(true)
        // getReq('referalanalysis', `?page=${currentPage + 1}&size=${currentEntry}&shop=${outletData[0]?.web_url}`)
        axios.get(`${SuperLeadzBaseURL}/referral/get_log/?page=${currentPage + 1}&size=${currentEntry}&shop=${outletData[0]?.web_url}`)
        .then((resp) => {
            console.log(resp)
