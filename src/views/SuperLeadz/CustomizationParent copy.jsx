...
import { AccordionBody, AccordionHeader, AccordionItem, Card, Container, DropdownItem, DropdownMenu, DropdownToggle, Modal, ModalBody, Row, UncontrolledAccordion, UncontrolledDropdown, Col, ModalHeader } from 'reactstrap'
...
const CustomizationParent = () => {
    ...
    const [finalObj, setFinalObj] = useState(EditThemeId ? {} : selectedThemeId !== "" ? { ...allPreviews[allPreviews?.findIndex($ => $?.theme_id === selectedThemeId)]?.object, campaignStartDate: moment(new Date()).format("YYYY-MM-DD HH:mm:ss") } : defaultObj)
    const [past, setPast] = useState([])
    const [future, setFuture] = useState([])
    const [themeName, setThemeName] = useState(EditThemeId ? "" : `Campaign-${generateRandomString()}`)
    const [defColors, setDefColors] = useState({})
    const [campaignLoader, setCampaginLoader] = useState(true)
    ...
    const updatePresent = (newState) => {
        console.log(past[past.length - 1], "past")
        const clonedFinalObj = JSON.parse(JSON.stringify(finalObj))
        setFinalObj(newState)
        const delay = 200
    ...
    const cancelAction = () => {
        ...
        if (EditThemeId) {
            ...
            const newObj = JSON.parse(EditThemeId)
                            ...
                            <AccordionBody accordionId='1'>
                                <div className='p-0 mx-0 my-1'>
                                    <div className='p-0 mb-2 justify-content-start align-items-center'>
                                        <span className='fw-bolder text-black' style={{ fontSize: "0.75rem" }}>Width Type:</span>
            ...
            styles = (
                <>
                    <UncontrolledAccordion defaultOpen={['1', '2', '3']} stayOpen>
                        ...
                        <AccordionItem>
                            <AccordionHeader className='acc-header' targetId='2' style={{ borderBottom: '1px solid #EBE9F1', borderRadius: '0' }}>
                                <p className='m-0 fw-bolder text-black text-uppercase' style={{ padding: "0.5rem 0px", fontSize: "0.75rem" }}>Alignment</p>
                            </AccordionHeader>
                            <AccordionBody accordionId='2'>
                                <div className='p-0 mb-1 align-items-center'>
                                    <span className='fw-bolder text-black' style={{ fontSize: "0.75rem" }}>Alignment:</span>
    ...
    const sendData = (e, actionType) => {
        ...
        if (themeName === "") {
            ...
            form_data.append("default_id", selectedThemeId)
            form_data.append("is_edit", EditThemeId ? 1 : 0)
    ...
    const saveDraft = async () => {
        ...
        form_data.append('shop', outletData[0]?.web_url)
        form_data.append('app', 'superleadz')
        const theme_id = await EditThemeId ? EditThemeId : (themeId !== "undefined" || themeId !== "null") ? Number(themeId) : 0
    ...
    //     const dupObj = { ...obj }
    //     dupObj?.responsive?.filter(item => item.position === "close")?.forEach(style => {
    //         style?.common?.forEach(value => {
    ...
    const getTemplate = () => {
        setCampaginLoader(true)
        axios(`${SuperLeadzBaseURL}/api/v1/pop_up_analytics/?shop=${outletData[0]?.web_url}&app=superleadz&theme_id=${EditThemeId}`)
        .then((response) => {
            const campaginData = JSON.parse(response?.data?.theme_json[0]?.custom_theme)
            console.log("res", campaginData)
            setFinalObj(campaginData)
            setDefColors(campaginData?.defaultThemeColors || {})
            setCurrPage(campaginData?.[`${mobileCondition}pages`][0].id)
            setThemeName(campaginData?.theme_name)
            setCampaginLoader(false)
            toDoOnload()
        })
        .catch((err) => {
            console.log(err)
    ...
    useEffect(() => {
        if (Object.keys(defColors).length !== 0) {
            const colWise = currPage === "button" ? [...finalObj?.[`${mobileCondition}button`]] : [...finalObj?.[`${mobileCondition}pages`][finalObj?.[`${mobileCondition}pages`]?.findIndex($ => $.id === currPage)].values]
            const newObj = { ...finalObj }
    ...
    // useEffect(() => {
    //     updatePresent({ ...finalObj, offerProperties: { ...finalObj?.offerProperties, colors: offerColors } })
    ...
    return (
        <Suspense fallback={null}>
            {
                campaignLoader ? <>
                    <div className='w-100 h-100 d-flex justify-content-center align-items-center my-5'><Spinner size={'30px'} /></div>
                </> : <>
                        ...
                        <div className="d-flex justify-content-center align-items-stretch border position-relative" style={{ height: "calc(100vh - 55px)" }}>
                            ...
                            <div className="d-flex align-items-stretch flex-grow-1 h-100">
                                            ...
                                            <UncontrolledAccordion stayOpen defaultOpen={["1", "2"]}>
                                                <AccordionItem>
                                                    ...
                                                    <AccordionBody accordionId='1'>
                                                        ...
                                                        <p className='m-0 fw-bolder text-black text-uppercase' style={{ padding: "0.5rem 0px", fontSize: "0.75rem" }}>Border and Shadow</p>
                                                        {currPage === "button" ? < BorderChange type={`btnStyles`} mainStyle={finalObj} setMainStyle={updatePresent} mobileCondition={mobileCondition} /> : <BorderChange type={`bgStyles`} mainStyle={finalObj} setMainStyle={updatePresent} mobileCondition={mobileCondition} />}
                                                    </AccordionBody>
                                                </AccordionItem>
                                                <AccordionItem>
                                                    <AccordionHeader className='acc-header' targetId='2' style={{ borderBottom: '1px solid #EBE9F1', borderRadius: '0' }}>
                                                    ...
                                                    <AccordionBody accordionId='2'>
                                                        <p className='m-0 fw-bolder text-black text-uppercase' style={{ padding: "0.5rem 0px", fontSize: "0.75rem" }}>Spacing</p>
                                                        <div className='p-0 mx-0 my-1'>
                                                            <InputChange parentType={currPage === "button" ? "btnStyles" : "bgStyles"} setMainStyle={updatePresent} mainStyle={finalObj} mobileCondition={mobileCondition} allValues={currPage === "button" ? finalObj?.backgroundStyles?.[`${mobileCondition}button`] : finalObj?.backgroundStyles?.[`${mobileCondition}main`]} type='padding' />
                                                        </div>
                                                        {currPage !== "button" && <div className='p-0 mx-0 my-1'>
                                                            <InputChange parentType={currPage === "button" ? "btnStyles" : "bgStyles"} setMainStyle={updatePresent} mainStyle={finalObj} mobileCondition={mobileCondition} allValues={currPage === "button" ? finalObj?.backgroundStyles?.[`${mobileCondition}button`] : finalObj?.backgroundStyles?.[`${mobileCondition}main`]} type='margin' />
                                                        </div>}
                                                    </AccordionBody>
                                                </AccordionItem>
                                            </UncontrolledAccordion>
                                                ...
                                                <AccordionItem className='bg-white border-bottom'>
                                                    ...
                                                    <AccordionBody accordionId='1'>
                                                        ...
                                                        {currPage === "button" && <div className='p-0 mx-0 mt-1 mb-2'>
                                                            ...
                                                            <InputChange parentType="btnStyles" setMainStyle={updatePresent} mainStyle={finalObj} mobileCondition={mobileCondition} hideLabel allValues={finalObj?.backgroundStyles[`${mobileCondition}button`]} type='margin' />
                                                        </div>}
                                                    </AccordionBody>
                                                </AccordionItem>
                                                {/* Position */}
                                ...
                                <div className="d-flex flex-column flex-grow-1 align-items-center bg-light-secondary">
                                    {returnRender({ outletData, slPrevBg, bgsettings: finalObj?.overlayStyles, currPage, setCurrPage, currPosition, setCurrPosition, indexes, setIndexes, popPosition: finalObj?.positions?.[`${mobileCondition}${pageCondition}`], bgStyles: finalObj?.backgroundStyles?.[`${mobileCondition}main`], crossStyle: finalObj?.crossButtons[`${pageCondition}`], values, setValues, showBrand, handleColDrop, handleDragOver, handleElementDrop, handleLayoutDrop, handleRearrangeElement, mouseEnterIndex, setMouseEnterIndex, mousePos, setMousePos, isEqual, makActive, colWise: currPage === "button" ? [...finalObj?.[`${mobileCondition}button`]] : [...finalObj?.[`${mobileCondition}pages`][finalObj?.[`${mobileCondition}pages`]?.findIndex($ => $.id === currPage)].values], setcolWise, setDragStartIndex, setDragOverIndex, isMobile, finalObj, setFinalObj: updatePresent, mobileCondition, openPage, setOpenPage, brandStyles, gotOffers, setTransfered, sideNav, setSideNav, btnStyles: finalObj?.backgroundStyles[`${mobileCondition}button`], offerTheme: finalObj?.offerTheme, navigate, triggerImage })}
                                </div>
                                {/* Theme Preview */}
                                {/* Edit Section */}
